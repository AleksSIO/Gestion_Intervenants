...
class Modele {
	...
	public function getIdInterventions() {
		if ($this->pdo != null) {
			$requete = "SELECT idintervention FROM intervention";
			$select = $this->pdo->query($requete);
			return $select->rowCount();
		} else {
			return null;
	...
	public function selectAllSalles($depart, $sallesParPage) {
		if ($this->pdo != null) {
			$requete = "SELECT * FROM salle ORDER BY idsalle LIMIT :depart, :sallesParPage";
			$select = $this->pdo->prepare($requete);
			$select->bindValue(':depart', $depart, PDO::PARAM_INT);
			$select->bindValue(':sallesParPage', $sallesParPage, PDO::PARAM_INT);
			$select->execute();
	...
	public function selectAllInterventions($depart, $interventionsParPage) {
		if ($this->pdo != null) {
			$requete = "SELECT * FROM intervention ORDER BY idintervention LIMIT :depart, :interventionsParPage";
			$select = $this->pdo->prepare($requete);
			$select->bindValue(':depart', $depart, PDO::PARAM_INT);
			$select->bindValue(':interventionsParPage', $interventionsParPage, PDO::PARAM_INT);
			$select->execute();
